...
/* 80013ac8       .text JMANewSinTable__FUc */
bool JMANewSinTable(u8 numBits)
{
	...
	for (u16 i = 0; i < jmaSinTableSize + (jmaSinTableSize / 4); i++) {
		jmaSinTable[i] = sinf(((M_PI * 2.0f) / jmaSinTableSize) * i);
	}
...
/* 80013c88       .text JMAEulerToQuat__FsssP10Quaternion */
...
/* 80013d40       .text JMAQuatLerp__FP10QuaternionP10QuaternionfP10Quaternion
 */
...
/* 80013de4       .text JMAHermiteInterpolation__Ffffffff */
                            ...
                            f32 time1, f32 value1, f32 tangent1)
{
	...
	return (value0 * ((1.0f + ((2.0f * f12) - (3.0f * f10)))))
	       ...
	       + (tangent0 * (f9 + (f11 - (2.0f * f2)))) + (tangent1 * (f11 - f2));
}
