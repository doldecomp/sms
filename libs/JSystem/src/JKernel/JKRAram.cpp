...
void JKRAram::changeGroupIdIfNeed(u8* data, int groupId)
{
	...
	if (currentHeap->getHeapType() == 'EXPH' && groupId >= 0) {
		JKRExpHeap::CMemBlock::getBlock(data)->newGroupId(groupId);
	}
}
                                     ...
                                     JKRHeap* heap, int id)
{
	...
	return mainRamToAram(buf, block->getAddress(),
                           ...
                           int id, u32* pSize)
{
	...
	if (expandSwitch == EXPAND_SWITCH_DECOMPRESS) {
		...
		JKRAramPcs(1, address, (uintptr_t)bufPtr, 32, nullptr);
	...
	if (compression == JKR_COMPRESSION_YAZ0) { // SZS
		...
		JKRAramPcs(1, address, (uintptr_t)szpSpace, p3, nullptr);
		if (p5 != 0 && p5 < expandSize)
			expandSize = p5;
		...
		changeGroupIdIfNeed(rv, id);
		JKRAramPcs(1, address, (uintptr_t)rv, p3, nullptr);
		if (pSize != nullptr) {
			*pSize = p3;
		}
		return rv;
                           ...
                           int id, u32* pSize)
{
	...
	p3 = p3 == 0 ? blockSize : p3;
	if (p4 + p3 > blockSize)
		p3 = blockSize - p4;

	return aramToMainRam(p4 + block->getAddress(), buf, p3, expandSwitch, p6,
	                     heap, id, pSize);
}
...
static u8* nextSrcData(u8* current)
{
	...
	JKRAramPcs(1, (uintptr_t)(srcAddress + srcOffset), ((uintptr_t)dest + left),
