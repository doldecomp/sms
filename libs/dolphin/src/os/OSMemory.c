...
static void MEMIntrruptHandler(__OSInterrupt interrupt, OSContext* context)
{
	...
	if (__OSErrorTable[OS_EXCEPTION_MAX]) {
		__OSErrorTable[OS_EXCEPTION_MAX](OS_EXCEPTION_MAX, context, cause,
		                                 addr);
		return;
	}

	__OSUnhandledException(OS_EXCEPTION_MAX, context, cause, addr);
}
