...
- Local changes have added 21,632 exactly matching code bytes and 125 matching functions/helpers across thirty-one batches; 76 verified game objects are linked from source.
...
3. More broadly, 104 unlinked game objects have 2,000 or fewer unmatched code bytes, totalling 94,780 bytes.
   Regenerate this list from `report.json` at each checkpoint rather than trusting these numbers.
   Batch 31 regeneration is saved in `docs/progress/GMSE01-completion-queue.json`: 104 authorized game files with 1..2,000 unmatched bytes, totaling 94,780 unmatched code bytes.
   This excludes THPPlayer despite the report classifying it as game code.
   Consult the batch 25 through batch 31 closure audits in `docs/progress/` before retrying stalled candidates; they record map gaps, shared issues, and rejected trials.
   ...
   Game coverage is 24.86575% matched / 2.8374174% source-linked; aggregate coverage is 38.48049% / 2.24211%.
   ...
   Further small multi-function candidates include `CameraSecureView` and `sunmgr`; revisit earlier deferrals when new evidence addresses their recorded blockers.
