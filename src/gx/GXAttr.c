                       ...
                       u32 pt_texmtx)
{
	...
	ASSERTMSGLINE(0x393, dst_coord < 8,
	              "GXSetTexCoordGen: Invalid coordinate Id");
	form = 0;
	row  = 5;
	switch (src_param) {
	case GX_TG_POS:
	...
	case 9:
		ASSERTMSGLINE(
		    0x3CF, src_param >= 12 && src_param <= 18,
	...
	switch (dst_coord) {
	case GX_TEXCOORD0:
		SET_REG_FIELD(0x402, gx->matIdxA, 6, 6, mtx);
		break;
	case GX_TEXCOORD1:
		SET_REG_FIELD(0x403, gx->matIdxA, 6, 12, mtx);
		break;
	case GX_TEXCOORD2:
		SET_REG_FIELD(0x404, gx->matIdxA, 6, 18, mtx);
		break;
	case GX_TEXCOORD3:
		SET_REG_FIELD(0x405, gx->matIdxA, 6, 24, mtx);
		break;
	case GX_TEXCOORD4:
		SET_REG_FIELD(0x406, gx->matIdxB, 6, 0, mtx);
		break;
	case GX_TEXCOORD5:
		SET_REG_FIELD(0x407, gx->matIdxB, 6, 6, mtx);
		break;
	case GX_TEXCOORD6:
		SET_REG_FIELD(0x408, gx->matIdxB, 6, 12, mtx);
		break;
	default:
		SET_REG_FIELD(0x409, gx->matIdxB, 6, 18, mtx);
		break;
	}
