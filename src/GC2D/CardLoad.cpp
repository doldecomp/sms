	...
	if (param_1 & 1) {
		...
		case 1:
			if (unk38->checkFrameMeaning(0x20)
				...
				SMSGetMSound()->startSoundSystemSE(MSD_SE_SY_CANCEL_COMMON, 0, nullptr, 0);
				unkB8 = 1;
				unk14 = 5;
			}
			break;
		...
		case 3:
			...
			if (unk18 >= 4) {
				if (unk18 >= 4 && unkBC >= 100 && gpCameraOption->unkA == 0
					...
					SMSGetMSound()->startSoundSystemSE(MSD_SE_SY_DECIDE, 0, nullptr, 0);
...
s8 TCardLoad::waitForChoice(TEProgress param_1, TEProgress param_2, int param_3)
{
	...
	case 2: {
		...
		if (old != unkB7) {
			...
			SMSGetMSound()->startSoundSystemSE(MSD_SE_SY_E3_MENU_CURSOR, 0, nullptr, 0);
...
s8 TCardLoad::selectFunction()
{
	...
	case 2:
		if (unk38->checkFrameMeaning(0x20)) {
			if (bVar1 == 0)
				SMSGetMSound()->startSoundSystemSE(MSD_SE_SY_DECIDE_COMMON, 0, nullptr, 0);
			else
				SMSGetMSound()->startSoundSystemSE(MSD_SE_SY_SELECT_COMMON, 0, nullptr, 0);
			unk10 = 3;
