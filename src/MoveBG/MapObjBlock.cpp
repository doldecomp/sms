...
void TSandBlock::control()
{
	...
	case TSandBlock::STATE_FALLING:
		...
		gpMSound->startSoundActor(MSD_SE_OBJ_SANDBLOCK_BREAK, &mPosition, 0, nullptr, 0, 0x4);
...
u32 TIceBlock::touchWater(THitActor* param_1)
{
	...
	if (gpModelWaterManager->checkFlagBottom4Bits(id, 0x1)) {
		...
		gpMSound->startSoundActor(MSD_SE_OBJ_ICE_BLOCK_MELT, &mPosition, 0, nullptr, 0, 4);
...
void TBrickBlock::kill()
{
	...
	gpMSound->startSoundActor(MSD_SE_OBJ_CLASSIC_BLOCK_B, &mPosition, 0, nullptr, 0, 4);
	SMSRumbleMgr->start(0x15, 0x14, &mPosition);
	appearObj(100.0f);
}
...
BOOL TSuperHipDropBlock::receiveMessage(THitActor* sender, u32 message)
{
	if (message == 3) {
		...
		gpMSound->startSoundActor(MSD_SE_OBJ_SUPERBLOCK_BREAK, &mPosition, 0, nullptr, 0, 4);
		return TRUE;
	}
	return FALSE;
}
