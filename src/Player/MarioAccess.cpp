...
bool SMS_IsMarioRoofing()
{
	...
	u32 val = gpMarioOriginal->mStatus & TMario::STATUS_TYPE_AND_ID_MASK;
...
{
	if (gpMarioOriginal->mStatus & TMario::STATUS_FLAG_SWIMMING) {
		return 1;
	} else {
		return 0;
	}
...
{
	if (gpMarioOriginal->mStatus & TMario::STATUS_FLAG_JUMPING) {
		return 1;
	} else {
		return 0;
	}
