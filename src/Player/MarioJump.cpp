...
void TMario::startJumpWall()
{
	...
	changePlayerStatus(0x02000886U, 0U, 0);
}
...
void TMario::doJumping()
{
	...
	if (mInput & 1) {
		...
		if (mAction == 0x88B && checkFlag(0x8000)) {
			if (mWaterGun->canSpray())
				mag = 2.5f * mIntendedMag;
		}

		if (mAction == 0x02000886U && mVel.y > 0.0f
...
int TMario::jumpingBasic(int statusOnGround, int animation, int processArg)
{
	...
	case 1: {
		...
		if (isStrong) {
			if (unk118 & 0x40000 ? true : false) {
				sinkInSandEffect();
				return changePlayerStatus(0x2033CU, 0U, 0);
			}
			if ((unk118 & 0x8000 ? true : false)
				...
				changePlayerStatus(0x479U, 0U, 0);
		...
		changePlayerStatus(statusOnGround, 0U, 0);
		if (!didTrample) {
			rumbleStart(0x14, (s16)mMotorParams.mMotorWall.get() / 2);
			stopVoice();
		}

		if (mPrevAction == 0x887U || mPrevAction == 0x895U
		    || mPrevAction == 0x896U)
			strongTouchDownEffect();
		else
	...
	case 2:
		if (mAction != 0x893U && mForwardVel > mDeParams.mClashSpeed.get()) {
			emitParticle(0xC);
			changePlayerDropping(0x208B0U, 0U);
			return result;
		}
		setAnimation(animation, 1.0f);
		if (onYoshi()) {
		...
		if (mWallPlane != nullptr) {
			if (mWallPlane->isNoWallJump()) {
				changePlayerStatus(0x88DU, 0U, 0);
				setPlayerVelocity(0.0f);
				break;
			}
		}
		if (mWallPlane != nullptr) {
			if (mWallPlane->isFence()) {
				...
				return changePlayerStatus(0x3000036CU, 0U, 0);
			}
		}
		if (mForwardVel > 16.0f && mAction != 0x88DU) {
			playerRefrection(0);
			mFaceAngle.y += 0x8000;
			if (mWallPlane != nullptr) {
				changePlayerStatus(0x8A7U, 0U, 0);
				if (isMario()) {
					rumbleStart(0x15, mMotorParams.mMotorWall.get());
			...
			if (mForwardVel >= 38.0f) {
				changePlayerStatus(0x208B0U, 0U, 0);
				break;
			}
			if (mForwardVel > 8.0f)
				setPlayerVelocity(-8.0f);
			return changePlayerStatus(0x208B6U, 0U, 0);
		}
		setPlayerVelocity(0.0f);
		break;
	...
	case 3:
		...
		changePlayerDropping(0x3800034BU, 0U);
		break;

	case 4:
		rumbleStart(0x15, mMotorParams.mMotorWall.get());
		changePlayerStatus(0x08200348U, 0U, 0);
		break;
	}
	return result;
}
...
int TMario::checkBackTrig()
{
	if (mInput & 0x8000) {
		...
		if (onYoshi() == 0) {
			...
			return changePlayerStatus(0x80088AU, 0U, 0);
		}
	}
	return 0;
}
...
int TMario::jumpDownCommon(int param_1, int animation, float velocity)
{
	...
	case 0:
		...
		break;
	case 1:
		changePlayerStatus(param_1, mActionArg, 0);
		break;
	case 2:
		setAnimation(2, 1.0f);
		playerRefrection(0);
...
BOOL TMario::catchStop()
{
	...
	case 0:
		...
		break;
	case 1:
		changePlayerStatus(0x0C000232U, 0U, 0);
		break;
	case 2:
		setPlayerVelocity(0.0f);
		break;
	}
	if (mActionState == 1 && isLast1AnimeFrame()) {
		mActionState = 2;
	}
	return 0;
}
...
int TMario::fireDowning()
{
	...
	case 1:
		if (mActionState < 2 && mVel.y < 0.0f) {
			...
			startVoice(0x7852);
			changePlayerStatus(0x08000239U, 0U, 0);
		}
		break;
	case 2:
		playerRefrection(0);
...
int TMario::thrownDowning()
{
	...
	case 1:
		if (mActionState < 2 && mVel.y < 0.0f) {
			...
			return changePlayerStatus(0x0C000223U, 0U, 0);
		}
		break;
...
{
	if (!checkFlag(0x8000))
		return changePlayerStatus(0x88DU, 0U, 0);
...
int TMario::hipAttacking()
{
	...
	case 1:
		if (mFloorPosition.y > mPosition.y) {
			...
			changePlayerStatus(0x80023CU, 0U, 0);
		}
		if (mActionTimer < 0x28) {
			f32 lift = (f32)(0x28 - mActionTimer) * 0.5f;
		...
		mVel.y = 0.0f;
		{
			...
			if (r == 1)
				return changePlayerStatus(0x0C000230U, 0U, 0);

			if (r == 2) {
				setPlayerVelocity(-16.0f);
				if (mVel.y > 0.0f)
					mVel.y = 0.0f;
				return changePlayerStatus(0x208B0U, 0U, 0);
			}
		}
		break;
	...
	case 2:
	case 3:
		setAnimation(0x3D, 1.0f);
		mActionTimer += 1;
		if (mActionTimer > mJumpParams.mSuperHipAttackCt.get())
			mActionState = 3;
		...
		if (r == 1) {
			...
			if (mGroundPlane->mActor != nullptr) {
				if (!onYoshi()
					...
					startVoice(0x78D3);
					return changePlayerStatus(0x200346U, 0U, 0);
				}
				if (mActionState == 2) {
...
{
	if (mario->mActionTimer == 0) {
		mario->mActionTimer += 1;
...
static int unknown_inline_6(TMario* mario)
{
	...
	if (mario->mActionState == 0 && mario->isLast1AnimeFrame())
		mario->mActionState = 1;

	if (mario->mActionState == 0)
...
static int unknown_inline_9(TMario* mario)
{
	if (mario->mInput & 0x8000)
		return mario->changePlayerStatus(0x80088AU, 0U, 0);
	...
	case 1: {
		...
		if (mario->mForwardVel < 0.0f)
			...
			nextStatus = 0x04000470;
		mario->changePlayerStatus(nextStatus, 0U, 0);
		break;
	}
	case 2:
...
static int unknown_inline_11(TMario* mario)
{
	...
	if (mario->mAction == 0x896U)
		mario->mModelFaceAngle = mario->mActionTimer * 4096;
	else
		mario->mModelFaceAngle = -(mario->mActionTimer * 4096);
...
{
	if (mario->mPrevAction == 0x10000358U) {
		if (mario->mActionArg == 0U)
			mario->setAnimation(0xF8, 1.0f);
		else
			mario->setAnimation(0x6F, 1.0f);
	} else {
		mario->setAnimation(0x4D, 1.0f);
	}
...
int TMario::jumpMain()
{
	...
	if (mHeldObject != nullptr) {
		if (mInput & 0x2000 ? true : false)
			changePlayerStatus(0x820008ABU, 0U, 0);
	}
