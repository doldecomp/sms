...
void TMareEventDepressWall::init3rdEvent()
{
	...
	TMapModelActor* actor
	    = static_cast<TMapModelActor*>(JDrama::TNameRefGen::search2("mareEP2"));
	if (actor) {
...
void TMareEventDepressWall::init2ndEvent()
{
	...
	initCommon();
	TMapModelActor* actor
	    = static_cast<TMapModelActor*>(JDrama::TNameRefGen::search2("mareEP1"));
	if (actor) {
		actor->setActor((MActor*)this);
		unk18[0] = 3600;
		for (int i = 1; i < unk10; ++i)
...
void TMareEventDepressWall::init1stEvent()
{
	...
	initCommon();
	TMapModelActor* actor
	    = static_cast<TMapModelActor*>(JDrama::TNameRefGen::search2("mareEP0"));
	if (actor) {
		actor->setActor((MActor*)this);
		unk18[0] = 3600;
		for (int i = 1; i < unk10; ++i)
