...
//                              unk48 is a TPosition3f -- see below
//
// The last of those is the only difference left, and it is a single word:
// `TSimpleEffect::unk48` must be a **TPosition3f**, not the TRotation3f the
// header now has. Both are 0x30 bytes and add no members, so neither the
// layout nor an instruction changes; what changes is the inline depth of the
// base-constructor chain. Retail's two branches (`new TEffectPinnaFunsui`,
// `new TEffectBiancoFunsui`, at .text 0x2014 and 0x208c) call
// `bl JGeometry::TRotation3<TMatrix34<SMatrix34C<f>>>::TRotation3()` and the
// linker keeps *this* TU's weak 4-byte copy of it (map: 0x802ac7e0, right
// after getNameRef_Enemy). With unk48 a TRotation3f the chain is
// factory(0) -> TEffectPinnaFunsui(1) -> TSimpleEffect(2) -> TRotation3(3),
// so MWCC refuses to expand one level lower and we emit
// `bl TMatrix34<SMatrix34C<f>>::TMatrix34()` instead -- the same empty 4-byte
// body under the wrong name, which objdiff still scores as a 100% match on the
// caller. With unk48 a TPosition3f, TPosition3's own empty constructor takes
// depth 3 and TRotation3's lands on the depth-4 refusal, which is exactly
// retail. Measured: the one-word change takes this unit from 99.95% to 100.00%
// code with data already at 100% (8832/8832, 5152/5152), passes
// validate-symbol-order, keeps the DOL at a6782903, and causes zero
// regressions in any other unit. It is not made here because EffectObj.hpp
// belongs to Enemy/effectObj.cpp. The same "extra derivation level" reasoning
// is already recorded on TBGBeakHit::unk74 in Enemy/BossGesso.hpp.
