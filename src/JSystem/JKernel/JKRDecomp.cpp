...
OSMessage JKRDecomp::sMessageBuffer[4]  = { nullptr };
...
void* JKRDecomp::run()
{
	...
	while (true) {
		...
		if (command->field_0x1c) {
			OSSendMessage(command->field_0x1c, (OSMessage)1,
			              OS_MESSAGE_NOBLOCK);
		} else {
                          ...
                          JKRDecompCommand::AsyncCallback callback)
{
	JKRDecompCommand* command
	    = new (JKRHeap::getSystemHeap(), -4) JKRDecompCommand();
...
bool JKRDecomp::sync(JKRDecompCommand* command, int isNonBlocking)
{
	...
	if (isNonBlocking == JKRDECOMP_SYNC_BLOCKING) {
		...
		result = OSReceiveMessage(&command->mMessageQueue, &message,
		                          OS_MESSAGE_NOBLOCK)
		         != FALSE;
	}
...
{
	JKRDecompCommand* command
	    = orderAsync(srcBuffer, dstBuffer, srcLength, dstLength, nullptr);
