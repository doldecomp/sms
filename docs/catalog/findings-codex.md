...
- `JSystem/JParticle/JPAEmitter`: Retail `.data` is 0x48 bytes of two jump tables. Source additionally emits a 12-byte `JSUIosBase` vtable from a shared inline destructor. Seven `createParticle` switch entries have shifted targets because the function body is nonexact. A unit-local data edit is not justified.
