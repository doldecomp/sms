...
5. **Named locals and statics.** Cases where the fix was a source-level declaration:
   ...
   - Open shared fixes: `MsAngleDiff`'s first parameter should be `const f32&` (the ROM re-reads `rot.x` after the inner `MsWrap`; by-value keeps it in `f31`), and `std::min`/`max` returning `const T&` are needed for `moveObject`'s clamp literals to land in `.sdata` rather than `.sdata2`.
