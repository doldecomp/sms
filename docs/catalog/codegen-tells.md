...
- **`MSound::startSoundActor` result binding: REJECTED, with the oracle that decides it.** At a site with no low-region slack, each 4-byte value an inlined callee binds pushes the caller's next temporary up by 4, so a byte-exact caller's temporary offset *counts* the inline's bindings: `TMapEventSink::control`'s vector sits at 0x30 = raw 0x28 + two bindings (`this`, `position`), and the result local would make 0x34. Retail's wrapper binds exactly two, so the 4 bytes the 37 other sites want come from a **second inlined level between those sites and the wrapper**, shaped like the emitted `TMario::startSoundActor(u32)` / `TMapObjBase::startSound(u16)` (research lead). An inlined wrapper's parameter binding always takes a slot; the same value named at the call site is register-allocated, so no raw spelling can imitate a wrapper's frame.
