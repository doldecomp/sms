...
- Promoted: `TMario::getM3UModel()` (kept at the MarioDraw/MarioCap sites that improved — `setAnimation` 94.8 -> 96.0, `calcAnim`, `initModel`, `addUpper`; reverted where an exact function was lost), `mHPMax` with `PARAM_INIT`, `SMSGetCameraOption()`, `SMSGetFlagManager()` (194 raw `getInstance()` sites remain for a per-site sweep, +4 each). Three named squares vs a summed expression is per site; no map support for a shared squared-distance helper (`TVec3::squared(const TVec3&)` with named squares costs `TShine::calc`).
