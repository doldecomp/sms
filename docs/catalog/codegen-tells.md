...
- **"x reloaded, y/z in callee-saved FPRs" is one recurring pattern**: a 12-byte stack local read twice with a call between, where retail CSEs `.y`/`.z` into f31/f30 and reloads `.x` (`TMapCollisionBase::updateTrans`, `CPolarSubCamera::updateDemoCamera_`, and batch 58's `calcAttackGuideAnm`/`TBGTentacleMtxCalc::calc`); frames short by the two FPR saves plus 24. Unexplained; research item.
