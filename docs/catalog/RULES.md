...
- **Named locals in an inlined callee are both a ranking lever and a slot**: two of them promote the value the last inline expansion consumes to r31 (`start`), and each dead one reserves 4 bytes of frame while an inlined-callee *parameter* reserves none; a **live** one (a named index in a fabricated accessor) is the honest form (frame-gaps.md: "Closure 220").
