...
- not: a defaulted **class** argument on `TVec3<f32>`'s copy constructor, `sub`, or any operator, to buy the `a = b - c` gap's missing 8 bytes above the temp: it lands retail's exact (4, 12) at 55 instructions and +9 functions, but MWCC mangles the parameter into the signature and the map fixes every candidate's arity (`__ct__...FRCQ29JGeometry8TVec3<f>` goes MISSING); the `operator-` half alone is +0/-2 and blocked only by the weak `__ami__` retail emits from two inline-refused sites in `TYoshiTongue::movement` (frame-gaps.md: "Research batch 213").
