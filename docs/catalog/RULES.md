...
- Inline-temp price is the callee's return type: reference 8, pointer 4, void 0, additive over the levels inside `operator-`; a consumed binding sits 4 below the temp, a dead one 4 above. Retail's `a = b - c` geometry is (4 below, 12 above), reachable only with two dead bindings: a +9-function bounty, not a lever; by-value returns still cost +6 (frame-gaps.md: "Research batch 142").
