...
- `interp->push()` vs `interp->push(TSpcSlice())` moves the slice temporary 4 bytes at equal frame (callee-level vs call-site temporary); retail's `ev*` slots mostly sit between them, so the missing depth is in `spcinterp.hpp`'s push chain (frame-gaps.md: "Research batch 298").
