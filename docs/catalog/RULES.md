...
- An UNUSED helper restored to its exact map size is a frame lever at every call site (MapObjBase `stopAnim` 0xc4: callers -8 each); a raw member read through its existing header accessor is +8 per read (frame-gaps.md: "Research batch 296").
