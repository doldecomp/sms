...
- In a ctor, `a.x = a.y = a.z = 0.0f;` stores descending where `set(0, 0, 0)` stores ascending; a `bool` helper of `if (c) return true;` chains is one `||` (shares `li r0, 1`, shrinks the UNUSED body to map size); emitter scale sets read through a `const TVec3&` (codegen-tells.md: "Closure 263").
