...
5. **Write the whole TU first, verify, commit.** Only then polish: at most about ten minutes per function below 99%, applying the catalog rules in this order: structure (calls, control flow, load/store order); inlining (depth counted from the innermost expression, the size allowance that shrinks with depth, the statement budget, wrappers/operators/trivial constructors as depth spacers, an UNUSED helper that exists to be the call or the level); then frame-gap causes (param `.get()` vs wrapper, accessor reads, uninitialised locals, named results).
